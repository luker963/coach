...
class Player:
    ...
    def __init__(self, x: int = 40, y: int = 40):
        self.x = x
        self.y = y
...
class FacEnv:
    ...
    def __init__(self):
        ...
        self.player = Player()
        # self.target = {'x': 2, 'y': 2}
        self.target = {'x': np.random.randint(0, 50), 'y': np.random.randint(0, 50)}
...
class TestCls(Environment):
    ...
    def _restart_environment_episode(self, force_environment_reset=False) -> None:
        ...
        self.env.target = {'x': np.random.randint(0, 50), 'y': np.random.randint(0, 50)}
        print("Wrongs: ", self.env.wrongs)
        print(self.env.target['x'], self.env.target['y'])
        self.env.wrongs = 0
        self.reward_limit = -50000
